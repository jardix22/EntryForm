(function(a){
	a(document).ready(function(c){
		...
		a.datepicker._defaults.dateFormat = 'yy-mm-dd';
		//a.datepicker._defaults.dateFormat = 'dd-mm-yy';

		a("#da-ex-accordion,#da-ex-accordion-plain").accordion();
		a("#da-ex-tabs,#da-ex-tabs-plain").tabs();
		a("#da-ex-datepicker,#da-ex-datepicker-0,#da-ex-datepicker-1,#da-ex-datepicker-inline").datepicker({
		...
		a("#da-ex-datepicker-year").datepicker({
			yearRange: "c-30:c+5",
			changeMonth: true,
            changeYear: true
		});
...
a("#da-ex-pba-trigger").bind("click",
 	function(d){
 		a(this).parent().next("#da-ex-pba").progressbar("value",
 ...
 d.preventDefault()}
		...
		a("#da-ex-dialog-form-div").dialog({
			...
			buttons:[{
				text: "Submit",
				click: function(){ a(this).find("form#da-ex-dialog-form-val").submit() }
			}]
		}).find("#da-ex-dialog-dp").datepicker({
			dateFormat:"yy-mm-dd"}).end().find("form").validate({
			rules:{
	...
	d.preventDefault()}
		...
		a("#da-ex-dialog-modal").bind("click",
		function(d){
			a("#da-ex-dialog-div").dialog("option",
			{
				modal:true}
