(function(a){
	a(document).ready(function(c){
		...
		a("div.da-panel.collapsible .da-panel-header .da-panel-toggler").live("click",function(f){
			...
			a(this).closest(".da-panel").children(":not(.da-panel-header)").slideToggle("normal",function(){
				if(!parentToggled){
					a(this).closest(".da-panel").toggleClass("collapsed");parentToggled=true
				}	
			});
			f.preventDefault()}
		);
	...
	a(this).addClass("active")}).bind("mouseup",function(f){
	a(this).removeClass("active")}).children(".da-header-dropdown").bind("mousedown",function(f){
		f.stopPropagation()
	});
	if(a.fn.placeholder){
	a("[placeholder]").placeholder()}a("table.da-table tbody tr:nth-child(2n)").addClass("even");a("table.da-table tbody tr:nth-child(2n+1)").addClass("odd");a("div#da-content #da-main-nav ul li a, div#da-content #da-main-nav ul li span").bind("click",function(f){
	...
	if(a.fn.tinyscrollbar){
		a(".da-panel.scrollable .da-panel-content").each(
			function(){
				var e=a(this).height(),
				f=a(this).children().wrapAll('<div class="overview"></div>').end().children().wrapAll('<div class="viewport"></div>').end().find(".viewport").css("height",e).end().append('<div class="scrollbar"><div class="track"><div class="thumb"><div class="end"></div></div></div></div>').tinyscrollbar({axis:"x"});
				
				a(window).resize(function(){f.tinyscrollbar_update()})
			}
		)
	}
	if(a.fn.tipsy){
		...
		for(var b in d){
			a(".da-tooltip-"+d[b]).tipsy({gravity:d[b]})
		}
		a("input[title], select[title], textarea[title]").tipsy({trigger:"focus",gravity:"w"})
	}
